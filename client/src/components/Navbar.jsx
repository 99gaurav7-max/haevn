...
export default function Navbar() {
  ...
  const linkClass = scrolled
    ? 'text-xs uppercase tracking-[0.25em] text-navy-500/60 hover:text-gold-500 transition-colors duration-300 font-medium relative after:absolute after:bottom-[-4px] after:left-0 after:h-[1px] after:bg-gold-500 after:transition-all after:duration-300 after:w-0 hover:after:w-full'
  ...
  const moreBtnClass = scrolled
    ? 'flex items-center space-x-1 text-xs uppercase tracking-[0.25em] text-navy-500/60 hover:text-gold-500 transition-colors duration-300 font-medium'
          ...
          <div className="flex items-center justify-between h-20">
                ...
                <div className={`absolute top-full left-0 mt-2 w-48 bg-warm-100/95 backdrop-blur-2xl border border-navy-500/10 transition-all duration-300 shadow-2xl shadow-black/10 ${
                  ...
                  {categories.slice(3).map((cat) => (
                    <Link key={cat.name} to={cat.path}
                      ...
                      className="block px-6 py-3 text-xs text-navy-500/50 hover:text-gold-500 hover:bg-navy-500/5 transition-colors duration-200 uppercase tracking-[0.15em]">
                      {cat.name}
                    </Link>
                  ))}
                </div>
            ...
            <div className="flex items-center space-x-1 sm:space-x-3">
              <button onClick={() => setSearchOpen(!searchOpen)} aria-label="Search"
                className={`btn-ghost text-xl ${scrolled ? 'text-navy-500/50' : 'text-white/50'}`}>
                <HiOutlineSearch />
              </button>
              {user ? (
                <div className="relative hidden sm:block"
                  ...
                  <div className={`absolute right-0 top-full mt-2 w-56 bg-warm-100/95 backdrop-blur-2xl border border-navy-500/10 transition-all duration-300 shadow-2xl shadow-black/10 ${
                    ...
                    <div className="px-6 py-3 border-b border-navy-500/8">
                      <p className="text-[10px] text-navy-500/40 uppercase tracking-widest">Welcome</p>
                      <p className="text-sm font-medium text-navy-500/80 mt-1">{user.name}</p>
                    </div>
                    <Link to="/profile" className="block px-6 py-3 text-xs text-navy-500/50 hover:text-gold-500 hover:bg-navy-500/5 uppercase tracking-[0.1em]">My Profile</Link>
                    <Link to="/profile?tab=orders" className="block px-6 py-3 text-xs text-navy-500/50 hover:text-gold-500 hover:bg-navy-500/5 uppercase tracking-[0.1em]">My Orders</Link>
                ...
                <Link to="/auth" className={`btn-ghost text-xl hidden sm:block ${scrolled ? 'text-navy-500/50' : 'text-white/50'}`}>
                  <HiOutlineUser />
                </Link>
              )}
              <button onClick={toggleCartDrawer} aria-label="Open cart"
                className={`btn-ghost text-xl relative ${scrolled ? 'text-navy-500/50' : 'text-white/50'}`}>
              ...
              <div className="max-w-4xl mx-auto px-4 py-6">
                <form onSubmit={handleSearch} className="relative">
                  <input
                    ...
                    className="w-full px-6 py-4 bg-warm-50/80 border border-gold-500/30 text-navy-500/90 placeholder:text-navy-500/25 focus:outline-none focus:border-gold-500/50 pr-14 text-lg font-light tracking-wide"
                    autoFocus
                  />
              ...
              <div className="space-y-1">
                {categories.map((cat) => (
                  <Link key={cat.name} to={cat.path}
                    ...
                    className="block px-4 py-3 text-sm text-navy-500/60 hover:text-gold-500 hover:bg-navy-500/5 transition-colors duration-200 uppercase tracking-[0.15em]">
                    {cat.name}
                  </Link>
                ))}
                ...
                {user ? (
                  <>
                    <Link to="/profile" onClick={() => setMobileOpen(false)}
                      className="block px-4 py-3 text-sm text-navy-500/60 hover:text-gold-500 uppercase tracking-[0.1em]">My Profile</Link>
                  ...
                  <Link to="/auth" onClick={() => setMobileOpen(false)}
                    className="block px-4 py-3 text-sm text-navy-500/60 hover:text-gold-500 uppercase tracking-[0.1em]">Sign In</Link>
                )}
              </div>
            </motion.div>
          </motion.div>
