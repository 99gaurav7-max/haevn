    ...
    return (
      <div className="pt-20">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <h1 className="text-3xl font-serif text-warm-50/90 mb-4">Order Confirmed</h1>
          <p className="text-warm-50/50 text-sm font-light mb-2">Thank you for your purchase.</p>
          <p className="text-warm-50/50 text-xs font-light mb-8">A confirmation has been sent to {form.email || user?.email}</p>
      ...
      <div className="border-b border-gold-500/10 bg-navy-600">
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-bold text-warm-50/90">Complete Your Order</h1>
          </motion.div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            ...
            <div className="mb-8 p-4 bg-gold-500/5 border border-gold-500/10 flex items-center gap-3">
              ...
              <p className="text-[10px] text-warm-50/60 tracking-wide">
...
step === i + 1 ? 'text-gold-500/80' : 'text-warm-50/50'
              ...
              {step === 1 && (
                <motion.div
                  ...
                  className="space-y-6"
                >
                  <h3 className="text-base font-serif text-warm-50/90 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <select disabled={submitting} required value={form.state} onChange={updateForm('state')} className="input-field">
                      ...
                      {indianStates.map(s => (
                        <option key={s} value={s} className="bg-navy-500">{s}</option>
                      ))}
                    </select>
                ...
                >
                    ...
                    {[
                      ...
                      <label key={id} className={`flex items-center gap-4 p-4 border cursor-pointer transition-all ${
                        form.paymentMethod === id
                          ...
                          : 'border-gold-500/10 bg-navy-500/80 backdrop-blur-xl hover:border-gold-500/20'
                      }`}>
                        <input type="radio" name="payment" value={id}
                          checked={form.paymentMethod === id}
                          onChange={updateForm('paymentMethod')}
                          className="accent-gold-500 w-4 h-4" />
                        <Icon className={`text-lg ${form.paymentMethod === id ? 'text-gold-500/80' : 'text-warm-50/50'}`} />
                        <span className="text-xs text-warm-50/60">{label}</span>
                      </label>
                    ))}
                  </div>
                  ...
                  {form.paymentMethod === 'upi' && (
                    <div className="mb-6">
                      ...
                      <p className="text-[9px] text-warm-50/40 mt-1">Enter your UPI ID to receive payment request</p>
                    </div>
                  )}
                  ...
                  {form.paymentMethod === 'cod' && (
                    <div className="mb-6 p-4 bg-gold-500/5 border border-gold-500/10">
                      <p className="text-xs text-warm-50/50 font-light">Pay with cash when your order is delivered. Please keep the exact amount ready.</p>
                    </div>
                  )}
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div className="p-6 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10 space-y-4">
                    <h4 className="text-[10px] text-gold-500/70 uppercase tracking-[0.25em] font-semibold">Shipping To</h4>
                    <p className="text-warm-50/70 text-xs">{form.firstName} {form.lastName}</p>
                    <p className="text-warm-50/40 text-xs font-light">{form.address}, {form.city}, {form.state} {form.zip}</p>
                  </div>

                  <div className="p-6 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10 space-y-3">
                    <h4 className="text-[10px] text-gold-500/70 uppercase tracking-[0.25em] font-semibold">Payment Method</h4>
                    <p className="text-warm-50/60 text-xs capitalize">{form.paymentMethod === 'upi' ? 'UPI — ' + form.upiId : form.paymentMethod === 'cod' ? 'Cash on Delivery' : 'Credit/Debit Card'}</p>
                  </div>
                  ...
                  <div className="p-6 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10 space-y-3">
                    ...
                    {cart.map((item, i) => (
                      <div key={i} className="flex justify-between text-xs">
                        <span className="text-warm-50/50 font-light">{item.name} × {item.quantity}</span>
                        <span className="text-warm-50/70">{formatINRDecimal(item.price * item.quantity)}</span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="sticky top-28 p-8 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10">
              ...
              <div className="space-y-3 mb-6">
                {cart.map((item, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="w-12 h-14 flex-shrink-0 bg-navy-500 overflow-hidden">
                      <img src={item.images?.[0]} alt="" className="w-full h-full object-cover"
                        onError={(e) => { e.target.style.display = 'none'; }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-warm-50/60 truncate">{item.name}</p>
                      <p className="text-[9px] text-warm-50/50 tracking-wide">{item.size} / {item.color} × {item.quantity}</p>
                      <p className="text-[10px] text-gold-500/80 font-medium mt-1">{formatINRDecimal(item.price * item.quantity)}</p>
                    </div>
                  </div>
                ))}
              </div>
              <hr className="border-gold-500/10 mb-4" />
              <div className="space-y-3 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-warm-50/70">{GSTAmount(gst)}</span>
                </div>
