      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-warm-50/70 text-sm leading-relaxed mb-10 font-medium">
              Our concierge team is available around the clock to assist with orders, styling advice, and any enquiries.
            </p>
            <div className="space-y-8 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10 p-10">
              {[
                ...
                <div key={label} className="flex items-start gap-4 group">
                  ...
                  <div>
                    <p className="text-[10px] text-warm-50/50 uppercase tracking-[0.2em] mb-1 font-medium">{label}</p>
                    <p className="text-sm text-warm-50/80 font-medium">{value}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.form initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="space-y-6 bg-navy-500/80 backdrop-blur-xl border border-gold-500/10 p-10">
            <h3 className="text-lg font-serif text-warm-50/90 font-semibold">Send a Message</h3>
            ...
            <select className="input-field w-full text-sm text-warm-50/50">
