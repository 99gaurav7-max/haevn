        ...
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <div className="h-4 w-24 bg-white/[0.02] animate-pulse" />
              <div className="h-10 w-3/4 bg-white/[0.02] animate-pulse" />
              <div className="h-6 w-32 bg-white/[0.02] animate-pulse" />
              <div className="h-24 bg-white/[0.02] animate-pulse" />
              <div className="h-12 bg-white/[0.02] animate-pulse" />
              <div className="h-12 bg-white/[0.02] animate-pulse" />
            </div>
          </div>
        </div>
    ...
    return (
      <div className="pt-20">
        <div className="max-w-8xl mx-auto px-4 py-20 text-center">
          <p className="text-white/40 text-lg font-light">Product not found</p>
    ...
    <div className="pt-20">
      ...
      <div className="border-b border-white/5 bg-[#05050f]/80">
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-[10px] text-white/30 tracking-wide">
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <motion.div
            ...
            className="space-y-4"
          >
            <div className="aspect-square bg-[#060610] overflow-hidden relative group">
          ...
          >
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-serif font-bold text-white/90">{product.name}</h1>
              <div className="flex items-center gap-4 mt-4">
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <HiStar key={i} className={`text-lg ${i < Math.floor(product.rating) ? 'text-gold-500/80' : 'text-white/10'}`} />
                  ))}
                </div>
                <span className="text-white/30 text-xs tracking-wide">{product.rating} ({product.reviews} reviews)</span>
              </div>
            </div>
            ...
            <div className="flex items-baseline gap-3">
              ...
              {product.originalPrice > product.price && (
                <>
                  <span className="text-xl text-white/20 line-through">{formatINR(product.originalPrice)}</span>
            ...
            <p className="text-white/50 leading-relaxed text-sm font-light tracking-wide">{product.description}</p>

            {/* Size */}
            <div>
              <h4 className="text-[10px] uppercase tracking-[0.25em] text-gold-500/70 font-semibold mb-3">
                Size <span className="text-white/20 normal-case font-light">— {selectedSize}</span>
              </h4>
            ...
            <div>
              <h4 className="text-[10px] uppercase tracking-[0.25em] text-gold-500/70 font-semibold mb-3">
                Color <span className="text-white/20 normal-case font-light">— {selectedColor}</span>
              </h4>
            ...
            <div>
              ...
              <div className="flex items-center border border-white/10 w-fit">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="px-4 py-3 text-white/30 hover:text-white/70 transition-colors">
                  <HiOutlineMinus />
                </button>
                <span className="px-6 py-3 text-white/70 border-x border-white/10 min-w-[3rem] text-center text-sm">{quantity}</span>
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button onClick={handleAddToCart}
                className={`btn-outline flex-1 flex items-center justify-center gap-2 text-xs py-4 tracking-[0.2em] transition-all ${
                  addedToCart ? 'bg-green-500/10 border-green-500/50 text-green-400' : ''
                }`}>
                <HiOutlineShoppingBag />
                {addedToCart ? 'Added!' : 'Add to Cart'}
              </button>
              <button className="w-14 h-14 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-400/70 hover:border-red-400/40 transition-all duration-300">
            ...
            <div className="pt-6 border-t border-white/5">
              ...
              <ul className="space-y-2">
                {product.details.map((detail, i) => (
                  <li key={i} className="text-white/40 text-xs flex items-start gap-2 font-light tracking-wide">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
              {[
                ...
                <div key={text} className="flex items-center gap-3 p-4 bg-white/[0.02] border border-white/5">
                  ...
                  <div>
                    <p className="text-[10px] font-medium text-white/60 uppercase tracking-wider">{text}</p>
        ...
        <section className="py-16 border-t border-white/5">
          <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div
              ...
              className="mb-10"
            >
              <h2 className="text-2xl md:text-3xl font-serif text-white/80">Complete the Look</h2>
